...
export const updateUserLimit = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { balanceLimit: limit }, { new: true });
        if (!user) {
            res.status(404).json({ error: "User not found" });
            return;
        }
...
export const blockUser = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { isBlocked: true, blockReason: reason || "Blocked by admin" }, { new: true });
        if (!user) {
            res.status(404).json({ error: "User not found" });
            return;
        }
...
export const unblockUser = async (req, res) => {
    try {
        const user = await User.findByIdAndUpdate(req.params.id, { isBlocked: false, blockReason: null }, { new: true });
        if (!user) {
            res.status(404).json({ error: "User not found" });
            return;
        }
