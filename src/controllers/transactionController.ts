...
// 🪙 Deposit initiation
export const depositUSDT = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) return res.status(404).json({ msg: "User not found", code: "40401" });
...
export const cancelDeposit = async (req: Request, res: Response) => {
  try {
    ...
    if (!rechargeNo) return res.status(400).json({ msg: "RechargeNo required", code: "40001" });
    ...
    if (!transaction) return res.status(404).json({ msg: "Transaction not found", code: "40401" });
...
// 💸 Withdraw
export const withdrawUSDT = async (req: Request, res: Response) => {
  try {
    ...
    if (rateSettings?.value?.USDT_INR) exchangeRate = rateSettings.value.USDT_INR;
...
// 🔄 Exchange USDT
export const exchangeUSDT = async (req: Request, res: Response) => {
  try {
    const { data } = req.body;
    ...
    if (rateSetting?.value?.USDT_INR) rate = rateSetting.value.USDT_INR;
...
export const addBankCard = async (req: Request, res: Response) => {
  try {
    ...
    if (exists) return res.status(400).json({ error: "Bank card already added" });
...
export const deleteBankCard = async (req: Request, res: Response) => {
  try {
    ...
    if (index === -1) return res.status(404).json({ error: "Bank card not found" });
