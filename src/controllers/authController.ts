...
// ✅ Verify Token
export const verifyTokenA = async (req: Request, res: Response) => {
  try {
...
export const verifyOTPAndSignup = async (req: Request, res: Response) => {
  try {
    ...
    const user: any = await User.findOne({ phone }).select("+otp +otpExpiry");
    ...
    if (inviteCode) {
      const referrer: any = await User.findOne({ inviteCode });
      if (referrer) {
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const user: any = await User.findOne({ phone, isVerified: true });
